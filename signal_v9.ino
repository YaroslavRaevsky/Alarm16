...
const char index_html[] PROGMEM = R"rawliteral( //объявление и инициализация строки с html страницей
<!DOCTYPE HTML><html><head> //тип документа для браузера
  <title>ALARM WiFi input</title> //заголовок страницы
  <meta name="viewport" content="width=device-width, initial-scale=20"> //размер и масштаб страницы

  <form action="/get" target="hidden-form"> //форма для заполнения имени сети
    WiFi name <input type="text" name="inputString1"> //запись имени сети в переменную
    <input type="submit" value="Submit WiFi name" onclick="document.location.reload(true)"> //кнопка для записи имени сети в переменную и перезагрузки страницы
    <p class="margin-bottom-20"></p>  //пробел
    saved WiFi name: %inputString1%   //вывод текущего имени сети на страницу
    <p class="margin-bottom-20"></p>  //пробел
    <p class="margin-bottom-20"></p>  //пробел
    <p class="margin-bottom-20"></p>  //пробел
  </form><br> //конец формы
  <form action="/get" target="hidden-form"> //форма для заполнения пароля сети
    Password <input type="text" name="inputString2"> //запись пароля сети в переменную
    <input type="submit" value="Submit Password" onclick="document.location.reload(true)"> //кнопка для записи пароля сети в переменную и перезагрузки страницы
    <p class="margin-bottom-20"></p>  //пробел
    saved Password: %inputString2%   //вывод текущего пароля сети на страницу
  </form><br> //конец формы

  <iframe style="display:none" name="hidden-form"></iframe> //оформление странцы
</body></html>)rawliteral"; //конец строки с html страницей
...
void setup() {   //действия, выплняемые однокрано при запуске микроконроллера
  mySwitch.enableReceive(digitalPinToInterrupt(4)); //выбор входа для считывания пакетов с приёмника 433 МГц
